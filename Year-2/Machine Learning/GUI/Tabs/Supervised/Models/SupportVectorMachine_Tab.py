...
from Workspace.Supervised.SupportVectorMachine import *

from itertools import permutations
from re import match

class SupportVectorMachineTab(ctk.CTkFrame):
...
#! ----- ----- ----- ----- ----- ----- ----- ----- <<<<< frame_01_01 >>>>> ----- ----- ----- ----- ----- ----- ----- -----
        ...
        self.kernel_options = ctk.CTkOptionMenu(self.frame_01_01, dynamic_resizing=False,
                                                       values=SVM_KERNELS, fg_color=MENU_FG, dropdown_fg_color=MENU_FG,
                                                       button_color=MENU_BTN, button_hover_color=MENU_BTN_HVR, dropdown_hover_color=MENU_BTN_HVR)
        self.kernel_options.grid(row=2, column=0, padx=(10, 5), pady=5, sticky=STICKY_X)
        self.kernel_options.set(KERNEL)

        self.c_entry = ctk.CTkEntry(self.frame_01_01, placeholder_text='C')
        self.c_entry.grid(row=3, column=0, padx=(10, 5), pady=5, sticky=STICKY_X)

        self.max_iterations_entry = ctk.CTkEntry(self.frame_01_01, placeholder_text='Max Iterations')
        self.max_iterations_entry.grid(row=2, column=1, padx=(5, 10), pady=5, sticky=STICKY_X)

        self.tolerance_entry = ctk.CTkEntry(self.frame_01_01, placeholder_text='Tolerance')
        self.tolerance_entry.grid(row=3, column=1, padx=(5, 10), pady=5, sticky=STICKY_X)
...
#! ----- ----- ----- ----- ----- ----- ----- ----- <<<<< frame_01_02 >>>>> ----- ----- ----- ----- ----- ----- ----- -----
        ...
        self.plot_machine_btn = ctk.CTkButton(self.frame_01_02, text='Plot Machine', state=DIS, command=self.plot_machine)
        self.plot_machine_btn.grid(row=2, column=0, columnspan=1, padx=(10, 5), pady=5, sticky=STICKY_X)

        self.plot_machine_axis = ctk.CTkOptionMenu(self.frame_01_02, dynamic_resizing=False, state=DIS,
                                                       values=[], fg_color=MENU_FG, dropdown_fg_color=MENU_FG,
                                                       button_color=MENU_BTN, button_hover_color=MENU_BTN_HVR, dropdown_hover_color=MENU_BTN_HVR)
        self.plot_machine_axis.grid(row=2, column=1, columnspan=1, padx=(5, 10), pady=5, sticky=STICKY_X)
        self.plot_machine_axis.set(PLOT_AXIS)
...
#! ----- ----- ----- ----- ----- ----- ----- ----- <<<<< Functions >>>>> ----- ----- ----- ----- ----- ----- ----- -----
    ...
    def write_model_report(self):
        self.write('Decision Tree Classification Report:')
        self.write(self.model.report)
    ...
    def test_model(self):
        ...
        try:
            ...
            for i in [self.report_or_variance_btn, self.plot_machine_btn, self.plot_machine_axis, self.confusion_or_regression_btn,
                      self.btn_01, self.btn_02, self.btn_03, self.btn_04]:
                i.configure(state=NRM)

    def plot_machine(self):
        xy = self.plot_machine_axis.get()
        if xy != PLOT_AXIS:
            xy = match(r"\('([^']*)',\s*'([^']*)'\)", xy)
            x = xy.group(1)
            y = xy.group(2)
            self.model.plot_machine(x, y)
...
#! ----- ----- ----- ----- ----- ----- ----- ----- <<<<< Regressor >>>>> ----- ----- ----- ----- ----- ----- ----- -----
    ...
    def varify_model(self):
        ...
        kernel         = self.kernel_options.get()
        ...
        if tolerance == '':
            ...
            try:
                if not 0.0 <= float(tolerance) <= 0.50:
                    self.write('Min Impurity Is Out Of Normal Range, Clipped To [0.00:0.50]')
        ...
        if all_ok:
            ...
            try:
                if model_type == CLASSIFICATION:
                    self.model = SVCModel(dataframe=self.dataset,
                                          label_column=label,
                                          C=float(c),
                                          kernel=kernel.lower(),
                    ...
                    self.model = SVRModel(dataframe=self.dataset,
                                          label_column=label,
                                          C=float(c),
                                          kernel=kernel.lower(),
                                          max_iter=int(max_iterations),
                ...
                for i in [self.model_type_btn, self.kernel_options, self.max_iterations_entry, self.label_options, self.data_split_options,
                          self.test_size_entry, self.random_state_entry, self.c_entry, self.tolerance_entry, self.varify_model_btn]:
                    i.configure(state=DIS)
                self.plot_machine_axis.configure(values=[str(i) for i in permutations([j for j in self.dataset.columns if j != label], 2)])
